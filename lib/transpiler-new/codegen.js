...
function emitSwitchStatement( node, options = {} ) {
	if ( options.asyncContext && !options.syncEval ) {
		const cases = node.cases.map( (section) => (
			`{ tests: [ ${section.values.map( (value, index) => `async function( __zuzu_switch_value ) { return __zuzu_truthy( ${emitSwitchTestExpression( section, index, node.comparator )} ); }` ).join( ', ' )} ], body: async function() ${emitBlock( section.consequent, {
				...options,
	...
	const cases = node.cases.map( (section) => (
		`{ tests: [ ${section.values.map( (value, index) => `function( __zuzu_switch_value ) { return __zuzu_truthy( ${emitSwitchTestExpression( section, index, node.comparator )} ); }` ).join( ', ' )} ], body: function() ${emitBlock( section.consequent, {
			...options,
			inSwitchSection: true,
			loopDepth: 0,
		} )} }`
