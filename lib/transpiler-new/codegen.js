...
function emitProgram( ast, options = {} ) {
	...
	const needsAsyncWrapper = programNeedsAsyncWrapper( ast )
		|| (
			options.asyncImports === true
			&& !syncEval
			&& programContainsImport( ast )
		);
...
function emitFunctionLike( node, header, options = {} ) {
	...
	const cleanup = cleanupNames
		.map( (name) => `__zuzu_maybe_demolish( ${name}, ${returnSlotName} );` )
		.join( '\n' );
	const bodyWrapper = [
		...
		returnSlotName
			? `if ( __zuzu_nonlocal && __zuzu_nonlocal.__zuzu_nonlocal_return ) { ${returnSlotName} = __zuzu_nonlocal.value; return ${returnSlotName}; }`
...
function emitBlock( node, options = {} ) {
	...
	const cleanup = cleanupNames
		.map( (name) => options.returnSlotName
			? `__zuzu_maybe_demolish( ${name}, ${options.returnSlotName} );`
			: `__zuzu_maybe_demolish( ${name} );` )
		.join( '\n' );
...
function emitSwitchStatement( node, options = {} ) {
	...
	if ( options.asyncContext && !options.syncEval ) {
		const cases = node.cases.map( (section) => (
			`{ tests: [ ${section.values.map( (value, index) => `async function( ${switchHereName} ) { return __zuzu_truthy( ${emitSwitchTest( section, index )} ); }` ).join( ', ' )} ], body: async function( ${switchHereName} ) ${emitSwitchBody( section.consequent )} }`
		) );
		const defaultBody = node.defaultCase
			? `async function( ${switchHereName} ) ${emitSwitchBody( node.defaultCase.consequent )}`
			: 'null';
	...
	const cases = node.cases.map( (section) => (
		`{ tests: [ ${section.values.map( (value, index) => `function( ${switchHereName} ) { return __zuzu_truthy( ${emitSwitchTest( section, index )} ); }` ).join( ', ' )} ], body: function( ${switchHereName} ) ${emitSwitchBody( section.consequent )} }`
	) );
	const defaultBody = node.defaultCase
		? `function( ${switchHereName} ) ${emitSwitchBody( node.defaultCase.consequent )}`
		: 'null';
	const tail = switchStatementContainsReturn( node )
		? options.inSwitchSection
			? 'if ( __zuzu_switch_result && __zuzu_switch_result.__zuzu_return ) { return __zuzu_switch_result; }'
		...
		if (
			...
			if ( node.callee.computed ) {
				const property = node.callee.property.type === 'BraceIdentifier'
					? `__zuzu_resolve_brace_key( __zuzu_receiver, ${JSON.stringify( node.callee.property.name )}, () => ${node.callee.property.name} )`
