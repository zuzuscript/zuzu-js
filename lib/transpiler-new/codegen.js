...
function marshalCaptureNames( node, options = currentEmitContext() ) {
	...
	function visit( value, parent = null ) {
		...
		if ( localKinds.has( value.type ) && value.id && value.id.name ) {
			declared.add( value.id.name );
		}
...
function createFunctionOptions( node, options = {} ) {
	...
	return {
		...
		asyncNames: new Set( [ ...( options.asyncNames || [] ), ...collectAsyncFunctionNames( node.body ? node.body.body : [] ) ] ),
		weakStorageNames: new Set( [
			...( options.weakStorageNames || [] ),
			...collectWeakDeclaredNames( node.body ? node.body.body : [] ),
		] ),
		scopeNames: new Set( [ ...( options.scopeNames || [] ), ...paramNames, ...extractSpecialPreludeNames( signature ) ] ),
		bodylessFunctionNames: new Set(),
	};
}
		...
		if ( param.type === 'SpecialParameter' ) {
			...
			if ( param.special === 'rest_only' ) {
				if ( param.containerType === 'PairList' ) {
					...
					lines.push( `if ( __a instanceof PairList ) { ${param.name} = __a; } else { throw new Exception( "named PairList parameter only accepts named arguments" ); }` );
					lines.push( '}' );
				}
				else {
					lines.push( `const ${param.name} = Array.prototype.slice.call( arguments, ${argIndex} );` );
	...
	switch ( node.type ) {
		...
		case 'ObjectExpression':
			return `( () => { const __zuzu_obj = {}; ${node.properties.map(
				(prop) => `__zuzu_obj[${JSON.stringify( prop.key )}] = __zuzu_retain_value( ${emitExpression( prop.value )} );`
...
function emitNewExpression( node ) {
	if ( node.callee.type === 'CallExpression' ) {
		...
		const args = emitCallArguments( node.callee.arguments ).join( ', ' );
		if (
...
function emitCallExpression( node, options = {} ) {
	...
	if ( node.callee.type === 'Super' ) {
		...
		source = `${emitExpression( node.callee )}( ${emitCallArguments( node.arguments ).join( ', ' )} )`;
	}
	...
	switch ( node.operator ) {
		case ':=':
			...
			if ( node.left && node.left.type === 'MemberExpression' ) {
				...
				return `__zuzu_assign_index( ${object}, ${key}, ${right}, ${node.isWeakWrite ? 'true' : 'false'} )`;
			}
			return `${left} = ${right}`;
		case '+=':
