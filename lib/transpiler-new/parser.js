...
function parse( tokens, options = {} ) {
	...
	function parseDeclarationBindingKeyAndName() {
		if ( current().type === 'identifier' ) {
			...
			if (
				...
				&& !( current().type === 'keyword' && current().value === 'but' )
			) {
	...
	function isSwitchComparatorToken( token ) {
		...
		return [
			...
			'eqi', 'nei', 'gti', 'gei', 'lti', 'lei', 'cmpi', 'in', '∈',
			'not_in', '∉', 'subsetof', '⊂', 'supersetof', '⊃',
			'equivalentof', '⊂⊃', 'does', 'can', '~', '@?', '∣',
			'divides', '∤',
	...
	function parseLogicalOr() {
		return parseLeftAssociative(
			parseOnlyif,
			(token) => token.type === 'keyword' && [ 'or', 'or?' ].includes( token.value )
	...
	function parseOnlyif() {
		return parseRightAssociative(
			parseLogicalXor,
			(token) => token.type === 'keyword' && [ 'onlyif', 'onlyif?' ].includes( token.value )
				|| ( token.type === 'operator' && [ '⊨', '⊨?' ].includes( token.value ) )
		);
	}
	...
	function parseLogicalXor() {
		return parseLeftAssociative(
			...
			(token) => token.type === 'keyword'
					&& [ 'xor', 'xor?', 'nor', 'nor?', 'xnor', 'xnor?' ].includes( token.value )
				|| ( token.type === 'operator'
					&& [ '⊻', '⊻?', '⊽', '⊽?', '↔', '↔?' ].includes( token.value ) )
		);
	}
	...
	function parseLogicalAnd() {
		return parseLeftAssociative(
			parseEquality,
			(token) => token.type === 'keyword'
					&& [ 'and', 'and?', 'nand', 'nand?', 'butnot', 'butnot?' ].includes( token.value )
				|| ( token.type === 'operator'
					&& [ '⋀', '⋀?', '⊼', '⊼?', '⊭', '⊭?' ].includes( token.value ) )
		);
	}
	...
	function startsNamedKeyBeforeColon() {
		return [ 'identifier', 'keyword', 'string' ].includes( current().type )
		...
		if (
			...
			if ( !matchPairListClose() ) {
				while ( true ) {
					...
					const start = current();
					const literalKey = startsNamedKeyBeforeColon()
						? parseNamedKeyBeforeColon()
						: null;
