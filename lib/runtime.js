...
async function runSwitchAsync( value, comparator, cases, defaultBody ) {
	...
	for ( const section of cases ) {
		...
		if ( sectionMatched || runNext ) {
			matched = true;
			const result = await taskRuntime.awaitValue( section.body( value ) );
			runNext = result === true;
	...
	if ( defaultBody && ( !matched || runNext ) ) {
		const result = await taskRuntime.awaitValue( defaultBody( value ) );
...
function operatorCardinality( value ) {
	...
	if (
		value
		&& value.constructor
		&& value.constructor.__zuzu_class_name
...
function zuzuCallMember( object, property, ...args ) {
	...
	if (
		object
		&& object.constructor
		&& object.constructor.__zuzu_class_name
		&& typeof object.__call__ === 'function'
	) {
		...
		const lastArg = fallbackArgs.length > 0
			? fallbackArgs[fallbackArgs.length - 1]
			: null;
		if (
			isPairListLike( lastArg )
			&& lastArg.__zuzu_temporary_call_args === true
		) {
			namedArgs = lastArg;
			fallbackArgs.pop();
		}
