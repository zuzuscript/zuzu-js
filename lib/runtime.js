...
function resolveCompatibilityPath( moduleName, hostName ) {
	if ( hostName === 'browser' ) {
		return moduleName;
	}
	const pathsModule = './' + 'paths';
	return require( pathsModule ).resolveCompatibilityPath( moduleName );
}
...
class ZuzuScript {
	...
	getModuleSearchRoots() {
		return [
			...
			this.host.resolve( this.repoRoot, 'stdlib', 'test-modules' ),
			this.host.resolve( this.repoRoot, 't', 'modules' ),
			this.host.resolve( this.repoRoot, 'stdlib', 'modules' ),
	...
	resolveModulePath( moduleName, fromFile ) {
		...
		candidates.push( resolveCompatibilityPath( moduleName, this.host.name ) );
		const withExt = [];
		...
		if ( resolved.endsWith( '.js' ) ) {
			...
			if (
				...
				const policy = {
					...
					load_module: ( name, requestFromFile = fromFile ) =>
						this.loadModule( name, requestFromFile || fromFile ),
