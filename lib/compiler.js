...
const jsStdRoot = paths.jsModuleRoot;
const pureStdRoot = paths.pureModuleRoot;
const browserBundlePath = path.join( paths.distRoot, 'zuzu-browser.js' );
...
function normalizedBrowserBundle() {
	if ( !fs.existsSync( browserBundlePath ) ) {
		throw new Error(
			`Missing browser bundle: ${browserBundlePath}. Run bin/zuzu-build-browser-bundle first.`
		);
	}
...
\t\tincludePaths: [ '/__zuzu_compiled__', '/modules' ],
